...
export const navigationTree:NavigationNode[]=[
  ...
  {id:"platform",label:"nav.platform",icon:"◇",roles:platformRoles,children:[
    ...
    {id:"productReviews",label:"nav.productReviews",icon:"·",routeName:"productReviews",accessKey:"platform.productReview",roles:["PLATFORM_ADMIN","PLATFORM_OPERATOR"]}
  ]},
  {id:"merchant",label:"nav.merchant",icon:"♙",roles:merchantRoles,children:[
    {id:"settings",label:"nav.storeProfile",icon:"·",routeName:"settings",accessKey:"merchant.store",roles:merchantRoles},
    {id:"members",label:"nav.members",icon:"·",routeName:"members",accessKey:"merchant.team",roles:merchantRoles}
  ]},
...
export function navigationForAccess(roles:UserRole[],routePermissions:AdminRoutePermission[]){
  ...
  const allowed=(node:NavigationNode)=>{if(node.id==="onboarding"&&isMerchant&&!isPlatformReviewer)return false;return(!node.roles||node.roles.some(role=>roles.includes(role)))&&(!node.accessKey||routePermissions.includes(node.accessKey));};
