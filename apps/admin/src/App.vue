...
const pageKeys: Record<string, MessageKey> = { overview:"overview",products:"products",inventory:"inventory",orders:"orders",afterSales:"afterSales",members:"members",reports:"reports",settings:"settings" };

async function loadUser() { if (!adminToken.value) return; try { currentUser.value = await apiRequest<CurrentUser>("/auth/me"); } catch { signOut(); } }
function authenticated(token: string) { adminToken.value = token; void loadUser(); }
...
function signOut() { sessionStorage.removeItem("mc-admin-token"); sessionStorage.removeItem("mc-admin-refresh"); adminToken.value = null; currentUser.value = null; }
onMounted(loadUser);
</script>
...
<template>
  ...
  <div v-else class="wrapper" :data-theme="themeAttribute">
    ...
    <main class="main-container">
      ...
      <MerchantOnboarding v-if="activePage === 'onboarding'" :roles="currentUser?.roles ?? []" />
      <section v-else-if="activePage === 'orders'" class="dashboard content-scroll"><div class="module-heading"><span>ORDER MANAGEMENT</span><h1>{{ t('orders') }}</h1><p>{{ t('comingSoonHint') }}</p></div><OrderBoard /></section>
      <section v-else class="placeholder content-scroll"><div class="placeholder-card"><span>◇</span><small>MOECRAFT CONSOLE</small><h1>{{ t(pageKeys[activePage] ?? 'overview') }}</h1><p>{{ t('comingSoonHint') }}</p><b>{{ t('comingSoon') }}</b></div></section>
    </main>
  </div>
</template>
