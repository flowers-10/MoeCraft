...
const pageKeys: Record<string, MessageKey> = { overview:"nav.overview",products:"nav.products",inventory:"nav.inventory",orders:"nav.orders",afterSales:"nav.afterSales",members:"nav.members",reports:"nav.reports",settings:"nav.settings" };

async function loadUser() { if (!adminToken.value) return; userLoading.value = true; try { currentUser.value = await apiRequest<CurrentUser>("/auth/me"); } catch { signOut(); } finally { userLoading.value = false; } }
function authenticated(token: string) { adminToken.value = token; userLoading.value = true; void loadUser(); }
...
function signOut() { sessionStorage.removeItem("mc-admin-token"); sessionStorage.removeItem("mc-admin-refresh"); adminToken.value = null; currentUser.value = null; userLoading.value = false; }
onMounted(loadUser);
</script>

<template>
  <AdminLogin v-if="!adminToken" @authenticated="authenticated" />
  <div v-else class="wrapper" :data-theme="themeAttribute">
    ...
    <main class="main-container">
      ...
      <MerchantOnboarding v-else-if="activePage === 'onboarding'" :roles="currentUser.roles" />
      <section v-else-if="activePage === 'orders'" class="dashboard content-scroll"><div class="module-heading"><span>ORDER MANAGEMENT</span><h1>{{ t('nav.orders') }}</h1><p>{{ t('common.comingSoonHint') }}</p></div><OrderBoard /></section>
      <section v-else class="placeholder content-scroll"><div class="placeholder-card"><span>◇</span><small>MOECRAFT CONSOLE</small><h1>{{ t(pageKeys[activePage] ?? 'nav.overview') }}</h1><p>{{ t('common.comingSoonHint') }}</p><b>{{ t('common.comingSoon') }}</b></div></section>
    </main>
  </div>
</template>
