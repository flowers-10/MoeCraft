...
function normalizedDelta() { const value = Number(delta.value); return Number.isFinite(value) ? Math.max(-props.item.available, Math.trunc(value)) : 0; }
function clampDelta() { delta.value = normalizedDelta(); }
function stepDelta(step: number) { delta.value = Math.max(-props.item.available, normalizedDelta() + step); }
function submit() { clampDelta(); if (delta.value === 0 && Number(threshold.value) === props.item.lowStockThreshold) return; emit("save", { delta: delta.value, reason: reason.value.trim(), lowStockThreshold: Number(threshold.value) }); }
  ...
  <div class="backdrop" @click.self="emit('close')">
    <aside class="drawer" role="dialog" aria-modal="true" :aria-label="t('inventory.drawerTitle')">
      ...
      <form v-if="canAdjust" @submit.prevent="submit">
        ...
        <div class="form-grid"><label><span>{{ t('inventory.delta') }}</span><div class="stepper"><button type="button" :disabled="Number(delta) <= -item.available" aria-label="减少库存" @click="stepDelta(-1)">−</button><input v-model="delta" type="number" :min="-item.available" step="1" @blur="clampDelta" @change="clampDelta"><button type="button" aria-label="增加库存" @click="stepDelta(1)">＋</button></div></label><label><span>{{ t('inventory.threshold') }}</span><UiInput v-model="threshold" class="threshold-field" type="number" min="0" /></label></div>
        <label><span>{{ t('inventory.reason') }}</span><UiInput v-model="reason" :placeholder="t('inventory.reasonPlaceholder')" /></label>
        <UiButton type="submit" :loading="busy" :disabled="Number(delta) === 0 && Number(threshold) === item.lowStockThreshold">{{ t('inventory.save') }}</UiButton>
      </form>
