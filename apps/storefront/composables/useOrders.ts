import type { AvailableCouponView, CheckoutQuote, OrderListItem, OrderView, PaymentView, SandboxPaymentResult, ShipmentTrackingView, ShippingAddressSnapshot } from "@moecraft/shared";
...
export function useOrders(){
  ...
  const tracking=(id:string)=>request<ShipmentTrackingView[]>(`/orders/${encodeURIComponent(id)}/tracking`);
  ...
  return{orders,pending,error,list,get,tracking,claimCoupon,claimedCouponIds,availableCoupons,quote,submit,cancel,confirmReceipt,startPayment,payment,closePayment,simulate};
}
