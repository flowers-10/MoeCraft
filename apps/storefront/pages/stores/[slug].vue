...
import type { CouponOfferView, PublicProductSearchResult, StoreProfileView } from "@moecraft/shared";
const route = useRoute();
const { request, mediaUrl } = useStorefrontCatalog();
const { locale } = useLocale();
const buyerApi = useOrders();
const session = useAuthSession();
const claimBusy = ref<string | null>(null);
...
const { data: coupons } = await useAsyncData(`store-coupons-${slug}`, () => request<CouponOfferView[]>(`/public/promotions/stores/${encodeURIComponent(slug)}`));
onMounted(async () => {
  if (session.accessToken.value) claimed.value = await buyerApi.claimedCouponIds().catch(() => []);
});
async function claim(couponId: string) {
  if (!session.accessToken.value) return navigateTo(`/login?redirect=${encodeURIComponent(route.fullPath)}`);
  claimBusy.value = couponId;
  try { await buyerApi.claimCoupon(couponId); if (!claimed.value.includes(couponId)) claimed.value.push(couponId); }
...
<template>
  <div class="store-page">
    ...
    <template v-else>
      ...
      <section v-if="coupons?.length" class="coupons"><div class="section-head"><h2>店铺优惠券</h2><span>领取后结算时直接选择</span></div><div class="coupon-list"><article v-for="coupon in coupons" :key="coupon.id"><strong>{{ coupon.type === 'FIXED' ? `¥${coupon.value}` : `${coupon.value}%` }}</strong><div><b>{{ coupon.name }}</b><small>满 ¥{{ coupon.minimumAmount }} 可用 · {{ coupon.productIds.length ? '指定商品' : '全店商品' }}</small></div><button :disabled="claimBusy === coupon.id || claimed.includes(coupon.id)" @click="claim(coupon.id)">{{ claimed.includes(coupon.id) ? '已领取' : claimBusy === coupon.id ? '领取中…' : '领取' }}</button></article></div></section>
...
.coupons{max-width:1144px;padding:30px 24px;margin:auto;border-bottom:1px solid #dde1e7}.coupon-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.coupon-list article{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:14px;padding:16px;border:1px solid #f0c7d2;border-radius:10px;background:#fff5f8}.coupon-list strong{color:#d14b6b;font-size:24px}.coupon-list article div{display:grid;gap:4px}.coupon-list small{color:#777e8b}.coupon-list button{padding:9px 14px;border:0;border-radius:6px;background:#d14b6b;color:#fff}.coupon-list button:disabled{opacity:.55}@media(max-width:700px){.coupon-list{grid-template-columns:1fr}}
</style>
