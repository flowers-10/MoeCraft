...
MoeCraft 的目标不是单店展示站，而是一个允许商家申请入驻、平台审核与治理、消费者购买二创及周边的多商家交易平台。最小可上线版本必须完成“商家入驻 → 商品审核上架 → 用户浏览下单 → 支付 → 商家履约 → 售后 → 平台结算”的闭环。
...
- 商品以 SPU 表达款式，以 SKU 表达可购买规格；二创预售需保存预计发售/补款/发货窗口。
...
- [x] 文件服务支持用途、所有者、MIME 白名单、大小上限、私有对象键和隔离状态；真实对象存储和扫描随首个上传业务接入。
...
- [ ] 资质和商品媒体使用签名对象存储、类型/大小校验和病毒扫描隔离；适配器随首个业务上传流程完成。
...
- [x] 库存调整必须填写原因，创建不可变流水；设置低库存阈值，并覆盖服务层测试。
- [x] 下单锁定、支付扣减、取消/超时释放；操作用版本号或条件更新防超卖。
- [x] 后台显示可售、锁定、总量及流水，不允许负库存；验证并发和回滚路径。

#### G17 `feat(storefront): deliver catalog browsing and product detail`

- [x] 首页楼层、类目页、搜索页、店铺页、商品详情页、SKU 选择和库存状态。
- [x] 筛选类目/品牌/IP/价格/现货或预售；排序相关度、新品、价格、销量。
- [x] 商品页包含预售说明、发货窗口、正版/版权字段、店铺信息、售后摘要。
- [x] 完成 skeleton、空态、错误态、移动端与 SEO metadata/结构化数据。

### Phase 3：交易最小闭环（G18–G25）

#### G18 `feat(cart): implement persistent multi-merchant cart`

- [x] 加购、改量、删除、选中、失效项、按店铺分组；服务端实时校验 SKU 与限购。
- [x] 登录时合并游客购物车，冲突采用明确规则并告知用户。
- [x] 购物车展示当前价，但声明最终价格以结算试算为准。

#### G19 `feat(user): implement profile and address book`

- [x] 资料、头像、语言/时区偏好、地址增删改查与默认地址。
- [x] 地址按国家/地区动态字段建模，手机号与邮编校验可配置。
- [x] 支持账号注销申请；高风险信息变更要求重新认证。
- [x] 完成 storefront 登录后闭环：登录成功优先返回经过校验的原目标页，否则进入账户概览；禁止外部 URL 重定向，已登录用户访问登录/注册页时返回合理业务页。
- [x] 首页与全站 Header 根据会话动态切换登录/注册和用户头像菜单；菜单至少提供账户概览、订单、地址、收藏、通知与安全设置入口，并支持明确的退出反馈。
- [x] 建立 `/account` 账户中心父布局和概览、资料、地址、安全等嵌套路由；订单、收藏、通知子路由分别由 G24/G30 接入，未完成前不得展示不可用的假入口。
- [x] 首次 SSR、客户端 hydration、刷新和直接访问私有深层链接时先完成会话恢复再渲染权限页面，提供加载态，避免登录闪烁、重复请求和首次点击路由失效。
- [x] 刷新令牌失效、账号禁用或 API 返回 401 时只执行一次会话清理并安全返回登录页；保留站内原目标地址，登录后可继续原流程。
- [x] 登录、退出及会话失效在多标签页同步；用户资料或语言偏好更新后 Header 和当前页面立即响应，不要求整页刷新。
- [x] 验收覆盖：游客首页、正常登录、带站内 redirect 登录、刷新账户深层路由、过期会话、退出、浏览器前进后退、多标签页退出及移动端账户菜单。

#### G20 `feat(promotion): implement coupon and simple pricing rules`

- [x] 首发仅做固定减免/百分比、最低金额、有效期、适用店铺/商品、总量/每人限领。
- [x] 结算时服务端重新计算；订单保存原价、优惠分摊和规则快照。
- [x] 后台支持创建、暂停、查看领取与使用统计；已使用记录不可删除。
...
- [x] 原子创建总单、商家子单、订单项快照、库存锁和支付意图；写入使用幂等键。
- [x] 状态至少：待支付、已支付/待发货、部分发货、已发货、已完成、已取消、售后中、已关闭。
- [x] 订单号不可猜测；买家/商家/平台数据域隔离；重复提交、超时和事务回滚有测试。
...
- [x] 队列任务关闭超时未支付订单、关闭支付单并释放库存。
- [x] 任务幂等，可重试且有死信；管理后台可查看失败任务但重放需高权限。
- [x] 对下单到支付全链路增加指标和告警。
...
- [x] 商家发货前复核订单状态；支持一个子单多个包裹和部分发货。
- [x] 物流公司、单号、发货时间、轨迹查询适配器；手工补录需审计。
- [x] 买家查看物流并确认收货；自动确认策略可配置。
...
- [x] 完成订单才可评价，限制每订单项一次；支持评分、文本、图片和追加评价预留。
- [x] 商家回复；平台按规则隐藏违规内容但保留审计记录。
- [x] 商品聚合评分异步更新，避免每次实时全表计算。
...
- [x] 登录与交易异常规则、IP/账号速率限制、重复下单与优惠滥用标记。
- [x] 平台可冻结商家销售或结算，但操作不能篡改历史订单。
- [x] 建立举报/申诉最小记录和处理时间线。
...
当前先按 G37 → G38 → G39 → G40 推进；G16–G36 的业务开发暂缓，幂等、并发、队列、对象存储和通知在对应业务模块中一并交付，不再单独拆横切任务。

#### G37 `test(core): add minimum critical-flow test harness`

- [x] API 已有认证、RBAC、商家数据域、商品状态机和错误契约的最小 `node:test` 基线。
- [x] 已覆盖认证会话轮换/退出、AuthorizationGuard 拒绝路径、商家入驻状态机和文件类型/所有者规则。
...
- [x] API 已有 request ID、结构化 HTTP 日志和基础敏感字段脱敏。
- [x] 已提供请求总数、错误数、状态分组、平均耗时指标，以及数据库探针 `/readiness`。
- [x] 支持 W3C trace context 和可选 HTTP telemetry exporter，输出请求 span 与脱敏异常事件。
- [x] 生成版本化 OpenAPI，契约变更可通过 `openapi:check` 校验。
- [x] 定义 API 可用性、P95、错误率、队列和支付回调的初始 SLO/告警口径。
- [x] 编写数据库、支付回调、超卖、队列和对象存储故障手册。

#### G39 `chore(release): create staging and deployment pipeline`

- [x] CI 依次执行 Format/Lint、typecheck、测试、build、OpenAPI/迁移兼容、依赖漏洞和密钥扫描。
- [x] 提供不可变 API 镜像、外置配置、staging compose、readiness gate 和快速回滚流程。
- [x] 提供数据库备份/恢复脚本、shadow database 迁移检查和演练记录要求。
- [ ] 在真实 staging 演练迁移、回滚、备份恢复；支付/退款、邮件和对象存储权限随对应 provider 接入后验收。

#### G40 `docs(launch): complete security privacy and launch checklist`

- [x] 完成威胁模型、权限矩阵、依赖/密钥扫描门禁与备份恢复演练记录模板；真实 staging 恢复仍是发布硬门禁。
- [x] 完成用户协议、隐私政策、商家协议、售后规则和二创/知识产权投诉流程草案，待法务确认后发布。
- [x] 完成容量 smoke、客服 SOP、财务对账 SOP、事故角色、RPO/RTO 和回滚决策规则；真实联系人须在内部 on-call 系统补齐。
- [x] 定义 internal → canary merchant → 5%/25%/50%/100% 灰度与自动阻断阈值；业务 provider 和交易闭环完成后执行。
...
## 9. 当前仓库基线审计（2026-07-21）

- `apps/storefront` 当前只有基础首页，商品、账户与交易页面均未实现。
- `apps/admin` 已形成商家入驻、店铺成员、目录和商品审核等角色化页面，交易工作台仍未实现。
- `apps/api` 已完成 G05–G15：认证会话、RBAC、审计、文件元数据、商家入驻、店铺成员、目录和商品审核上架。
- API 当前已具备全局 DTO 校验、统一响应、稳定错误码、请求 ID、结构化 HTTP 日志、指标、W3C trace context、可选错误导出和优雅关闭；生产告警接线仍需部署环境配置。
- `packages/shared` 已承担 API 响应、错误码、状态机、权限和领域视图契约。
- 当前用 Node 20 内置 `node:test` 建立了 15 项认证、RBAC、商家数据域、商品/入驻状态机、readiness、metrics、trace、文件和错误契约基线；订单/支付/库存数据库集成、幂等和并发路径随业务模块补齐。
...
| M6 工程门禁 | G37–G40 | 测试、可观测性、发布、合规与演练 | 否 |
| M7 生产上线 | G37–G40 + G16–G36 | 交易闭环、生产演练、合规与灰度发布 | 是 |
