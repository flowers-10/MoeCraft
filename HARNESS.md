...
- API 当前已具备全局 DTO 校验、统一响应、稳定错误码、请求 ID、结构化 HTTP 日志和优雅关闭；完整指标、Trace、错误追踪和生产告警仍待 G38。
- `packages/shared` 已承担 API 响应、错误码、状态机、权限和领域视图契约。
- 当前用 Node 20 内置 `node:test` 建立了认证、RBAC、商品状态机和错误契约的最小测试基线；数据库集成、跨商家越权和并发路径仍待 G37。
