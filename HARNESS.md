...
- [ ] 为三端建立类型化环境变量入口和 `.env.example`；示例值不得伪装成可用生产密钥。
- [ ] 启动时校验数据库、JWT、站点 URL、CORS、对象存储等必填项。
- [ ] 将客户端可见变量与服务端密钥严格区分。

#### G03 `feat(shared): define domain primitives and api contract`

- [ ] 建立 ID、Money、Currency、Locale、分页、排序、错误响应、日期字符串等共享类型。
- [ ] 建立用户、商家、商品、订单、支付、售后的状态枚举和允许迁移说明。
- [ ] 建立权限常量，禁止散落字符串。

#### G04 `chore(api): add database migration and seed workflow`

- [ ] 选定并固定数据库（当前示例为 MySQL）与 Prisma 版本；提交 schema 和迁移，禁止提交本地数据库。
- [ ] seed 仅创建安全演示账号、类目和商品；密码通过环境变量或一次性生成。
- [ ] 文档化 migrate、seed、reset（仅本地）命令。

### Phase 1：平台基础能力（G05–G10）

#### G05 `feat(api): implement authentication and sessions`

- [ ] 注册、登录、退出、刷新、当前用户、忘记/重置密码。
- [ ] 登录失败返回统一错误，刷新令牌轮换、撤销与设备会话列表。
- [ ] 添加限流、密码策略和安全审计事件。

#### G06 `feat(frontends): add auth flows and route guards`

- [ ] storefront：注册/登录/找回密码/会话过期处理。
- [ ] admin：登录、无权限页、按身份落到平台或商家工作台。
- [ ] 验收未登录、过期令牌、禁用账号、多标签页退出场景。

#### G07 `feat(platform): establish i18n and locale persistence`

- [ ] storefront 与 admin 接入 i18n，建立中英词典、语言选择器与缺词回退。
- [ ] API 仅返回稳定错误码，必要的邮件/通知模板按 locale 渲染。
- [ ] 对日期、金额、数字和校验错误做本地化；CI/脚本检查词典 key 一致性。

#### G08 `feat(api): implement rbac and merchant data scope`

- [ ] 预置 `CUSTOMER`、`MERCHANT_OWNER`、`MERCHANT_STAFF`、`PLATFORM_OPERATOR`、`PLATFORM_ADMIN`。
- [ ] Guard 同时检查权限和 `merchantId` 数据域；平台跨商家查询需要独立权限。
- [ ] 建立角色分配规则，禁止普通管理员自我提权。

#### G09 `feat(frontends): add typed api clients and error handling`

- [ ] 两前端统一 base URL、认证、刷新并发锁、超时、请求取消和错误映射。
- [ ] 添加全局错误页、404、维护态、toast 与表单字段错误映射。
- [ ] 传递 locale、timezone、requestId；敏感错误不直接展示堆栈。

#### G10 `feat(api): add audit log and file service foundation`

- [ ] 审计日志覆盖登录、权限、商家审核、商品审核、库存调整、退款、结算、系统配置。
- [ ] 文件服务支持用途、所有者、MIME、大小、状态和签名访问。
- [ ] 管理后台提供只读审计检索，审计日志不可由普通管理员修改。

### Phase 2：商家入驻、目录与商品（G11–G17）

#### G11 `feat(merchant): implement merchant onboarding`

- [ ] API：申请草稿、提交、补件、通过、拒绝、撤回状态机；保存审核意见与时间线。
- [ ] admin 商家视角：分步表单、资质上传、协议确认、进度查看。
- [ ] admin 平台视角：审核队列、资料查看、拒绝原因模板、启用/停用。
- [ ] 限制重复申请；资质文件私有；审核全程记录审计。

#### G12 `feat(merchant): implement store profile and staff`

- [ ] 店铺名、slug、logo、banner、介绍、客服信息、退货地址、营业状态。
- [ ] 邀请成员、角色分配、移除成员；店主不可被最后一个移除。
- [ ] storefront 店铺主页仅展示审核通过且营业中的店铺。

#### G13 `feat(catalog): implement categories brands and franchises`

- [ ] 平台维护多级类目、品牌、作品/IP、角色、标签、属性模板和中英翻译。
- [ ] 防止类目循环；被商品引用的数据禁用而非直接删除。
- [ ] storefront 输出可索引的类目/品牌/IP 聚合页。

#### G14 `feat(product): implement product drafts and sku variants`

- [ ] 商家创建商品草稿：多语言标题/描述、类目、品牌/IP/角色、材质、比例、厂商、版权信息。
- [ ] SKU 支持规格组合、商家编码、条码、重量尺寸、价格和初始库存。
- [ ] 图片排序、封面、alt、视频预留；保存草稿不要求满足上架全部字段。

#### G15 `feat(product): add product review and publishing workflow`

- [ ] 状态：草稿 → 待审核 → 已通过/已拒绝 → 在售/下架/归档。
- [ ] 修改关键字段后重新审核；平台可给字段级意见；商家可复制商品。
- [ ] API 阻止未审核、店铺停用或无库存 SKU 对外销售。
